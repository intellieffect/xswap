...
- A `codex` entry that reaches *another* install's `xswap-codex` is treated as connected instead of as a Codex release. Only the proxy in the record was ever compared, so a second install ahead on PATH (a project venv, pipx beside uv) was wrapped as if it were a release: `auto-enable --wrap-codex` rewrote the record to `realCodex: <the other xswap-codex>` -- erasing the release path from `auto.json` -- and the unattended shadow repair (every launch, `list`, `use` and alert tick) did the same on its own. Plain `codex` then exec'd `xswap-codex`, which exec'd itself: `codex --version` never returned, while `xswap doctor` named an xswap-codex as the real Codex and `auto-disable` would have "restored" `codex` to it. `Manager.codex()` -- the binary behind every xswap launch -- applies the same rule.
...
- `auto-status`'s `codexWrapped` follows the PATH lookup, a relative PATH entry ahead of the wrapped one included: `codexWrapped` is then `false` with `wrapperReason: relative-path-entry`. That reason classifies the PATH element, not the recorded entry's link state, so it is not one of `wrapper_drift`'s (the recorded entry can be perfectly connected), and both fields describe the working directory `auto-status` itself ran in. With automatic switching disabled the recorded entry's own reason still wins, as before.
- An empty PATH element is one of those relative entries: POSIX reads it as the working directory (both bash and zsh run `./codex` for it, and `export PATH="$UNSET_VAR:$PATH"` writes one), so it is reported as `./codex` instead of being dropped before the walk. A relative element whose `codex` does reach `xswap-codex` is not a bypass -- plain `codex` there goes through xswap -- so `codexWrapped` stays `true`, `use`/`switch` print nothing, and `xswap doctor` is WARN naming the entry and the directory it depends on, instead of FAIL claiming it is "not xswap-codex". xswap never *runs* a relative `codex` either: `Manager.codex()` -- the binary behind `xswap login`, `xswap run`, a bridged session, `read_limits`, and the pool `auto-enable` builds -- falls back to the recorded `realCodex`, or refuses when no record can resolve it, rather than execing whatever file the current directory holds with a pool account's `CODEX_HOME`.
- A `wrapper.path` recorded before 0.8.0 can itself be relative (0.7.8 stored `str(shutil.which('codex'))` verbatim), and nothing migrates it. It is now the named skip `relative-record`: no launch, `list`/`usage`, `use`, `auto-disable`, or `relocate-codex` resolves it against the working directory, which re-pointed -- and later "restored" -- a `codex` inside whatever repository xswap happened to run in while the entry it really wrapped kept running `xswap-codex` with no record left. `xswap doctor` FAILs with the manual repair (point the entry that still runs `xswap-codex` back at the real Codex, then re-run `auto-enable --wrap-codex` from a shell whose PATH holds no relative entry), `auto-status` reports `wrapperReason: relative-record`, and `auto-disable` names the record it left untouched.
