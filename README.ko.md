...
리셋권은 `codex reset credits: N available`로 별도 표시하며, 서버가 제공한 사용 가능한 리셋권의 만료일도 보여줍니다. 정보를 제공하지 않으면 `unknown`으로 표시합니다(0개와 구분). `usage --json`에는 `resetCredits`가 포함됩니다. 조회는 리셋권을 사용하지 않습니다.
...
CLI 계정 전환 로그는 실행 중인 TUI 화면에 직접 출력하지 않으며 `xswap auto-status`에서 확인할 수 있습니다. 연결 오류 안내는 TUI 종료 후 출력합니다.
