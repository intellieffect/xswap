...
자동 모드 CLI 세션이 끝나면(`/exit`, Ctrl-D) xswap이 `xswap: Session ended. Resume this conversation:` 한 줄과 실제로 동작하는 `env … xswap run --auto … -- resume ID` 명령 하나를 출력하고, 이미 사라진 소켓을 가리키는 Codex 자체의 `--remote` 꼬리말(`Reconnect: codex --remote unix:///tmp/xs-…/rpc.sock resume ID`)은 제거합니다. Codex의 토큰 사용량 줄과 나머지 출력은 그대로입니다. TUI에 실제 터미널을 그대로 주려면 `XSWAP_RAW_EXIT=1`을 설정하십시오 — [docs/session-switching-details.ko.md](docs/session-switching-details.ko.md) 참고.
