...
from xswap.manager import (
    parse_cache_seconds,  # import-time copies: not patch targets today (see cli/__init__ docstring)
)
...
def add_auto_status_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
...
def add_auto_tick_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
...
def add_auto_enable_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
...
def add_auto_policy_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
...
def add_auto_disable_parser(sub: argparse._SubParsersAction) -> argparse.ArgumentParser:
    return sub.add_parser("auto-disable", help="Disable auto defaults and restore the codex symlink")


def run_auto_status(args: argparse.Namespace, manager: Manager) -> None:
    from xswap.codex_cli import show_status
    show_status(manager, args.prune)


def run_auto_tick(args: argparse.Namespace, manager: Manager) -> int:
...
def run_auto_enable(args: argparse.Namespace, manager: Manager) -> None:
    from xswap.codex_cli import enable
    enable(manager, args.accounts, args.wrap_codex)


def run_auto_policy(args: argparse.Namespace, manager: Manager) -> None:
    from xswap.codex_cli import set_policy
    set_policy(manager, args.weekly_remaining)


def run_auto_disable(args: argparse.Namespace, manager: Manager) -> None:
