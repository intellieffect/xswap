...
`xswap.codex_cli.<name>`, which is where they used to live. See `_late`.
"""
...
from xswap.live import AccountPool, LiveError, validate_threshold
from xswap.fsutil import atomic_json
from xswap.paths import ROOT_VARIABLE, default_root, settings_path
from xswap.settings import read_settings


def _late(name):
    """Resolve NAME through `xswap.codex_cli`, which re-exports it, at call time.

    These functions used to be defined in `codex_cli`, and the test suite still
    patches them there (`patch('xswap.codex_cli.swap_symlink')`,
    `patch('xswap.codex_cli.entry_drift')`). Calling the local definition would
...
def reconnect_wrapper(manager):
    ...
    """
    from xswap.relocate import canonical_codex_path, inside_root
