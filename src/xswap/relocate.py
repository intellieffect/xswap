...
def canonical_codex_path(manager, path, accounts=None):
    ...
    """
    from xswap.codex_cli import link_target_path
    literal = Path(path)
...
def relocate(manager, dry=False):
    ...
    from xswap.manager import SwapError, atomic_json
    from xswap.codex_cli import read_settings, status_data, swap_symlink
