...
def fallback_warning(rows, settings, now=None, lang="en", shape=None):
    ...
    if not others:
        return None
    for name in others:
        left = summary(by_name[name], now, lang, shape)['remaining']
...
def render(rows, settings, include_spark=False, details=False, color=None, now=None, lang="en", sessions=None,
    ...
    for index, row in enumerate(rows, 1):
        ...
        if details and is_ok(row['status']) and credit_lines is not None:
            # Whatever extra credit lines this platform has; core does not know of any.
            lines.extend('  ' + line for line in credit_lines(row.get('resetCredits')))
        if details:
            buckets = [b for b in row['buckets'] if include_spark or not is_extra(b)]
            lines.extend('  ' + line for line in usage_lines(buckets))
        elif include_spark:
            buckets = [b for b in row['buckets'] if is_extra(b)]
    ...
    warning = fallback_warning(rows, settings, now, lang, shape)
    if warning:
...
def dashboard(manager, lang="en"):
    ...
    provider = getattr(manager, "provider", None) or providers.get()
    shape = provider.quota_shape
