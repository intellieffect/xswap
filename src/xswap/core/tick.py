...
turn start, and new sessions start on whatever `xswap use` last chose. This
...
def describe_shortfall(row, reserve, shape=None):
    ...
    left = weekly_left(row, shape)
    if left is not None and left <= reserve:
        return f"{row['name']} weekly {percent(left)} left, at or below the {reserve:g}% reserve"
    limit = quota.reached(row["buckets"], shape)
    if limit:
        return f"{row['name']} limit reached ({limit})"
    # The bucket names itself in the sentence, so the line reads the same for every
    # platform without this module knowing any of their names.
    buckets = quota.quota_buckets(row["buckets"], shape)
...
def decide(rows, current, pool, reserve, failed=frozenset(), shape=None):
    ...
    if current in failed or row["status"] in SIGN_IN_STATUSES:
        ...
        state = buckets_available(row["buckets"], None, reserve, shape)
    else:
        state = None
    if state is True:
...
def run_tick(manager, dry_run=False, max_age=None, json_output=False):
    ...
    if json_output:
        ...
        from xswap.core.json_output import auto_tick_payload
