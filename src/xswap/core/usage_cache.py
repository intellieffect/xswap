...
`identity` is reached through `hooks`, which resolves it in `xswap.manager`'s
namespace at call time. The quota read itself goes through
`manager.provider.read_usage`, which uses the same hooks, so the suite's
...
class UsageCache:
    ...
    def account_usage(
        ...
        if disabled:
            ...
            # offline views say so without spawning the platform's CLI and without reusing an
            # older cache entry. A live read (no --cached) still tries again; a success clears it.
            ...
            if cached is not None:
                ...
                try:
                    snapshot = self.manager.provider.read_usage(self.manager, home)
    ...
    def best_account(
        ...
        be protected by the live auto bridge.
        """
