...
than imported here, so existing patches still bite. Anything that depends on
which platform an account belongs to (where its home lives, what that home is
called, what makes a fresh one usable) comes from `self.manager.provider`.
"""
...
class Registry:
    ...
    def provider_of(self, entry: AccountRecord) -> str:
        ...
        must never rewrite them to add one: a plain `xswap list --offline` would
        otherwise rewrite `accounts.json` on a machine that only reads it. The
        default is the one provider this build selects, which is what every
        existing record was.
        """
    ...
    def register(self, name: str, home: str | Path | None = None) -> Path:
        ...
        with self.manager.locked():
            ...
            if any(a["home"] == str(home) for a in data["accounts"].values()):
                raise SwapError(f"This {self.manager.provider.home_label} is already registered under another name.")
            data["accounts"][name] = {"home": str(home), "managed": False, "provider": self.manager.provider.name}
    ...
    def prepare(self, name: str) -> Path:
        ...
        with self.manager.locked():
            ...
            home = self.manager.provider.account_home(self.manager.root, name)
            ...
            self.manager.provider.prepare_home(self.manager, home, self.manager.source, data["accounts"])
            data["accounts"][name] = {"home": str(home), "managed": True, "provider": self.manager.provider.name}
            atomic_json(self.path, data)
        return home
    ...
    def purge_target(self, name: str, entry: AccountRecord) -> Path:
        ...
        state root, a hand-edited `home` -- the purge reported "Deleted the managed
        ...
        if recorded != self.manager.provider.account_home(self.manager.root, name):
