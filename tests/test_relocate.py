...
class RelocateTests(TestCase):
    ...
    def test_cli_launch_links_the_runtime_home_to_the_reference_packages(self):
        ...
        with patch.object(self.manager, 'codex', return_value='/fixture/codex'), patch('xswap.codex_cli.serve_cli', fake_serve), \
                patch('xswap.plugins.ensure_plugins'), contextlib.redirect_stderr(io.StringIO()):
    ...
    def test_relocate_moves_releases_links_runtime_and_rewrites_auto_json(self):
        ...
        ref = self.source / 'packages' / 'standalone'
        with patch('xswap.manager.Manager', return_value=self.manager), contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_refuses_when_the_recorded_codex_runs_through_a_current_it_cannot_place(self):
        ...
        with patch('xswap.manager.Manager', return_value=self.manager), contextlib.redirect_stderr(io.StringIO()) as err:
    ...
    def test_refuses_without_a_reference_home_outside_the_root(self):
        ...
        with patch('xswap.manager.Manager', return_value=nested), contextlib.redirect_stderr(io.StringIO()) as err:
            self.assertEqual(main(['relocate-codex']), 1)
