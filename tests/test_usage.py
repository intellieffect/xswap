...
''')
    ...
    def test_timeout_reaps_its_child(self):
        ...
        def start(*args, **kwargs):
            ...
            return child
        with patch("xswap.usage.subprocess.Popen", side_effect=start), self.assertRaisesRegex(UsageError, "timed out"):
    ...
    def test_reset_credits_survive_cache(self):
        ...
        with patch("xswap.manager.read_limits", return_value=payload) as fetch, patch.object(manager, "codex", return_value="codex"):
    ...
    def test_detailed_list_and_json_include_reset_credits(self):
        ...
        payload = response()
        payload["rateLimitResetCredits"] = {"availableCount": 2, "credits": None}
        with patch("xswap.manager.read_limits", return_value=payload), patch.object(manager, "codex", return_value="codex"):
    ...
    def test_list_fetches_only_signed_in_profiles_and_keeps_active_selection(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits", return_value=response()) as fetch, patch.object(manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as output:
        ...
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as full:
    ...
    def test_offline_never_starts_a_usage_server(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits") as fetch, contextlib.redirect_stdout(io.StringIO()):
            manager.show_accounts(offline=True)
        fetch.assert_not_called()

    def test_per_account_error_does_not_hide_other_accounts(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits", side_effect=UsageError("service unavailable")), patch.object(manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as output:
            manager.show_accounts(json_output=True)
    ...
    def test_main_list_warn_returns_three_and_prints_to_stderr(self):
        ...
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(Manager, "codex", return_value="codex"), \
    ...
    def test_short_omits_disabled_registry_entries(self):
        ...
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as output:
            manager.show_accounts(short=True)
        self.assertEqual(output.getvalue(), "*main 77/1\n")

    def test_short_composes_with_offline(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits") as fetch, contextlib.redirect_stdout(io.StringIO()) as output:
    ...
    def test_second_call_within_max_age_serves_cache_without_spawning_codex(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits", return_value=response()) as fetch, patch.object(manager, "codex", return_value="codex"):
    ...
    def test_failed_fetch_is_never_cached(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits", side_effect=UsageError("boom")), patch.object(manager, "codex", return_value="codex"):
            rows = manager.account_rows(max_age=60)
        self.assertIn("usage unavailable", rows[0]["status"])
    ...
    def test_json_row_reports_cached_field_both_ways(self):
        manager = self.manager()
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as out:
            manager.show_accounts(json_output=True)
    ...
    def test_offline_never_consults_or_populates_the_cache(self):
        ...
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(manager, "codex", return_value="codex"):
            manager.account_rows(max_age=60)
        with patch("xswap.manager.read_limits") as fetch:
    ...
    def test_main_list_cached_and_warn_together_reports_cached_rows_below_threshold(self):
        ...
        with patch("xswap.manager.read_limits") as fetch, patch.object(Manager, "codex", return_value="codex"), \
             patch.dict(os.environ, env), contextlib.redirect_stdout(io.StringIO()), \
             contextlib.redirect_stderr(io.StringIO()) as stderr:
    ...
    def test_service_rejected_login_is_recorded_in_a_private_state_file(self):
        ...
        with patch("xswap.manager.read_limits", side_effect=UsageError("sign in again to read usage")), patch.object(manager, "codex", return_value="codex"):
    ...
    def test_other_fetch_failures_leave_no_state_record(self):
        ...
        for error in (UsageError("usage service unavailable"), UsageError("usage request timed out"), UsageError("update Codex CLI to read usage")):
            with self.subTest(error=str(error)), patch("xswap.manager.read_limits", side_effect=error), patch.object(manager, "codex", return_value="codex"):
