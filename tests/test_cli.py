...
class CliTests(TestCase):
 ...
 def test_codex_update_that_replaced_the_link_is_reconnected_on_next_use(self):
  ...
  from xswap.manager import plain_codex_notice
  def which(name):return str(proxy if name=='xswap-codex' else cli)
 ...
 def drift(self):
  from xswap.codex_cli import wrapper_drift
 ...
 def test_wrapper_drift_names_every_skip_reason_and_reconnect_stays_silent(self):
  ...
  from xswap.codex_cli import DRIFT_REASONS, status_data
  real,updated,proxy,cli=self.wrapped_fixture()
  def which(name):return str(proxy if name=='xswap-codex' else cli)
  with patch('shutil.which',side_effect=which),contextlib.redirect_stdout(io.StringIO()),contextlib.redirect_stderr(io.StringIO()) as err:
   enable(self.manager,'main,second',wrap=True)
 ...
 def test_describe_drift_pairs_cause_and_fix_without_secrets(self):
  from xswap.codex_cli import DRIFT_REASONS, describe_drift, wrapper_drift
 ...
 def test_bridged_session_end_reconnects_after_in_session_update(self):
  ...
  with patch('shutil.which',side_effect=which),contextlib.redirect_stdout(io.StringIO()),contextlib.redirect_stderr(io.StringIO()) as err:
   enable(self.manager,'main,second',wrap=True)
   with patch('xswap.codex_cli.serve_cli',fake_serve),patch('xswap.plugins.ensure_plugins'):
 ...
 def test_codex_main_reports_missing_real_codex_explicitly(self):
  ...
  from xswap.codex_cli import codex_main
  calls=[]
  with patch.dict(os.environ,{'CODEX_SWAP_HOME':str(self.manager.root),'CODEX_HOME':str(self.source)}),patch('xswap.codex_cli.sys.argv',['xswap-codex','--version']),patch('xswap.codex_cli.os.execve',side_effect=lambda *a:calls.append(a)),contextlib.redirect_stderr(io.StringIO()) as err:
 ...
 def test_codex_main_passes_through_when_real_codex_exists(self):
  ...
  with patch.dict(os.environ,{'CODEX_SWAP_HOME':str(self.manager.root),'CODEX_HOME':str(self.source)}),patch('xswap.codex_cli.sys.argv',['xswap-codex','--version']),patch('xswap.codex_cli.os.execve',side_effect=fake_execve),contextlib.redirect_stderr(io.StringIO()):
   codex_main()
 ...
 def test_auto_status_codex_wrapped_follows_the_path_lookup(self):
  from xswap.codex_cli import status_data
  self.shadow_fixture()
 ...
 def test_codex_path_entries_orders_classifies_and_dedupes(self):
  from xswap.codex_cli import codex_path_entries
  self.shadow_fixture()
 ...
 def test_scan_survives_record_removed_by_another_sweep(self):
  ...
  stale,_=self.make_run('stale',updated_at=time.time()-8*86400)
  with patch('xswap.codex_cli.os.open',side_effect=FileNotFoundError),contextlib.redirect_stdout(io.StringIO()) as out:
   show_status(self.manager)
  report=json.loads(out.getvalue())
 ...
 def test_prune_survives_rmtree_errors(self):
  ...
  with patch('xswap.codex_cli.shutil.rmtree',side_effect=FileNotFoundError),contextlib.redirect_stdout(io.StringIO()) as out:
   show_status(self.manager)
  self.assertTrue(run_dir.exists())
  self.assertEqual(json.loads(out.getvalue())['pruned'],0)
...
class PassthroughTests(TestCase):
 ...
 def run_wrapper(self,args,extra_env=None):
  ...
  with patch('xswap.manager.Manager',return_value=self.manager),patch.dict(os.environ,environ),patch('sys.argv',['xswap-codex',*args]),patch('xswap.codex_cli.os.execve',side_effect=lambda path,argv,env:calls.append((path,argv,env))),contextlib.redirect_stderr(io.StringIO()) as err:
 ...
 def test_rejected_login_falls_back_with_the_login_command(self):
  ...
  from xswap.manager import identity
  self.wrap()
 ...
 def test_interactive_invocations_still_take_the_bridge(self):
  ...
  self.wrap()
  with patch('xswap.manager.Manager',return_value=self.manager),patch.dict(os.environ,{}),patch('sys.argv',['xswap-codex','hello']),patch('xswap.codex_cli.launch_cli',return_value=0) as bridge,patch('xswap.codex_cli.os.execve') as execve:
 ...
 def test_run_upgrade_launches_into_the_profile_home_instead_of_the_bridge(self):
  ...
  self.mapped_project('second')
  calls=[]
  with patch('shutil.which',side_effect=self.which),patch('xswap.codex_cli.launch_cli') as bridge,patch('xswap.manager.subprocess.call',side_effect=lambda command,env:calls.append((command,env)) or 0):
...
class PassthroughDocsTests(TestCase):
 ...
 def test_both_readmes_say_where_run_upgrade_installs_the_release(self):
  ...
  for name,marker,required in self.cases:
   blocks=[block for block in (Path(__file__).parent.parent/name).read_text().split('\n\n') if marker in block]
...
class DocPromiseTests(TestCase):
 ...
 def test_readmes_promise_only_what_the_code_does(self):
  ...
  for name,phrase in self.overpromises:
   self.assertNotIn(phrase,(Path(__file__).parent.parent/name).read_text(),name)
...
class ResumeHomeTests(TestCase):
 ...
 def setUp(self):
  ...
  home_patch=patch('xswap.codex_cli.Path.home',return_value=self.base/'user')
 ...
 def test_explicit_resume_uses_original_home_without_copying(self):
  from xswap.codex_cli import resume_home
