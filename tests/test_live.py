...
class PoolPrepareTests(unittest.TestCase):
    ...
    def test_quota_outage_is_optional_only_when_asked(self):
        pool = self.make_pool()
        with patch('xswap.live.check_file_store', create=True), \
                patch('xswap.manager.check_file_store'), patch('xswap.live.read_auth'), \
                patch('xswap.live.load_credentials', return_value={'accessToken': 'x'}), \
                patch('xswap.live.read_limits', side_effect=UsageError('usage request timed out')):
...
class PoolAuthStateTests(unittest.TestCase):
    ...
    def test_fresh_rejection_is_recorded_and_raised_as_sign_in_required(self):
        with patch('xswap.live.read_limits', side_effect=UsageError('sign in again to read usage')) as fetch:
            with self.assertRaises(SignInRequired) as caught:
    ...
    def test_recorded_rejection_skips_the_probe_even_when_quota_is_optional(self):
        ...
        with patch('xswap.live.read_limits') as fetch:
    ...
    def test_record_for_a_previous_login_label_is_ignored_and_swept_by_a_success(self):
        ...
        with patch('xswap.live.read_limits', return_value=limits()) as fetch:
    ...
    def test_outage_keeps_the_record_untouched(self):
        with patch('xswap.live.read_limits', side_effect=UsageError('usage request timed out')):
            with self.assertRaises(UsageError):
                self.pool.prepare('main')
...
class BridgeLogFileTests(unittest.TestCase):
    ...
    def test_cap_keeps_only_the_newest_lines(self):
        ...
        with patch('xswap.live.BRIDGE_LOG_MAX_BYTES', 64), patch('xswap.live.BRIDGE_LOG_KEEP_LINES', 3):
