...
class ResolveXswapTests(unittest.TestCase):
    def test_prefers_which_result_resolved(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("xswap.alert.shutil.which", return_value=str(link)):
                self.assertEqual(resolve_xswap(), str(target.resolve()))

    def test_falls_back_to_argv0_only_when_it_is_named_xswap(self):
        with patch("xswap.alert.shutil.which", return_value=None), \
    ...
    def test_a_relative_which_result_is_skipped_for_the_argv0_fallback(self):
        ...
        with patch("xswap.alert.shutil.which", return_value="bin/xswap"), \
             patch.object(sys, "argv", ["/some/dir/xswap"]):
            self.assertEqual(resolve_xswap(), str(Path("/some/dir/xswap").resolve()))
...
class InstallUninstallStatusTests(unittest.TestCase):
    def setUp(self):
        ...
        self.home.mkdir()
        home_patch = patch("xswap.alert.Path.home", return_value=self.home)
        home_patch.start()
        self.addCleanup(home_patch.stop)
        darwin_patch = patch("xswap.alert.sys.platform", "darwin")
        darwin_patch.start()
        self.addCleanup(darwin_patch.stop)
        which_patch = patch("xswap.alert.shutil.which", return_value="/opt/homebrew/bin/xswap")
        which_patch.start()
        self.addCleanup(which_patch.stop)

    def test_dry_run_install_writes_nothing_and_never_calls_launchctl(self):
        with patch("xswap.alert.subprocess.run") as run, contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_install_writes_run_sh_and_plist_and_bootstraps(self):
        ...
        with patch("xswap.alert.subprocess.run", side_effect=responses) as run, \
    ...
    def test_install_bakes_the_state_root_the_job_must_read(self):
        ...
        with patch("xswap.alert.subprocess.run", side_effect=responses), \
    ...
    def test_install_replaces_a_running_run_sh_instead_of_rewriting_it(self):
        ...
        with patch("xswap.alert.subprocess.run", side_effect=[completed(returncode=1), completed(returncode=0)]), \
             contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_uninstall_removes_plist_and_alert_dir_and_calls_bootout(self):
        ...
        with patch("xswap.alert.subprocess.run", side_effect=install_responses), \
             contextlib.redirect_stdout(io.StringIO()):
            install(self.root)
        self.assertTrue(plist_path().exists())
        with patch("xswap.alert.subprocess.run", return_value=completed(returncode=1, stderr="not loaded")) as run, \
             contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_uninstall_dry_run_changes_nothing(self):
        ...
        with patch("xswap.alert.subprocess.run", side_effect=install_responses), \
             contextlib.redirect_stdout(io.StringIO()):
            install(self.root)
        with patch("xswap.alert.subprocess.run") as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_status_reports_present_and_loaded_with_log_tail(self):
        ...
        with patch("xswap.alert.subprocess.run", return_value=completed(returncode=0)) as run, \
             contextlib.redirect_stdout(io.StringIO()) as out:
            code = status(self.root)
        self.assertEqual(code, 0)
    ...
    def install_ok(self, **kwargs):
        responses = [completed(returncode=1), completed(returncode=0)]  # not already loaded, then bootstrap ok
        with patch("xswap.alert.subprocess.run", side_effect=responses), contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_install_auto_switch_writes_tick_step_and_status_reports_it(self):
        ...
        with patch("xswap.alert.subprocess.run", return_value=completed(returncode=0)), contextlib.redirect_stdout(io.StringIO()) as status_out:
            status(self.root)
    ...
    def test_uninstall_notes_that_auto_mode_is_untouched_when_the_tick_step_was_installed(self):
        self.install_ok(auto_switch=True)
        with patch("xswap.alert.subprocess.run", return_value=completed(returncode=1, stderr="not loaded")), \
             contextlib.redirect_stdout(io.StringIO()) as out:
...
class NonDarwinTests(unittest.TestCase):
    def setUp(self):
        ...
        self.home.mkdir()
        patcher = patch("xswap.alert.Path.home", return_value=self.home)
        patcher.start()
        self.addCleanup(patcher.stop)
        platform_patch = patch("xswap.alert.sys.platform", "linux")
        platform_patch.start()
        self.addCleanup(platform_patch.stop)
        # Deterministic regardless of what's actually on this test runner's PATH.
        which_patch = patch("xswap.alert.shutil.which", return_value="/usr/local/bin/xswap")
        which_patch.start()
        self.addCleanup(which_patch.stop)
    ...
    def test_uninstall_exits_2_without_writing(self):
        with patch("xswap.alert.subprocess.run") as run:
            code = uninstall(self.root)
        self.assertEqual(code, 2)
        run.assert_not_called()
...
class MainCLIWiringTests(unittest.TestCase):
    ...
    def test_dry_run_install_wires_through_main_without_touching_launchctl(self):
        with patch("xswap.alert.sys.platform", "darwin"), \
             patch("xswap.alert.shutil.which", return_value="/opt/homebrew/bin/xswap"), \
             patch("xswap.alert.subprocess.run") as run, \
             contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_status_wires_through_main(self):
        with patch("xswap.alert.sys.platform", "darwin"), contextlib.redirect_stdout(io.StringIO()) as out:
