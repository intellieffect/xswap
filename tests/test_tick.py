...
class AutoTickCommandTests(unittest.TestCase):
    ...
    def tick(self, mapping, argv=(), report=REPORT):
        ...
        with patch("xswap.manager.Manager", return_value=self.manager), \
             patch("xswap.manager.read_limits", side_effect=fake) as fetch, \
             patch.object(Manager, "codex", return_value="codex"), \
             patch("xswap.switch.switch_running", return_value=report) as broadcast, \
    ...
    def test_auth_failed_hook_excludes_candidates(self):
        ...
        with patch("xswap.tick.is_auth_failed", side_effect=lambda manager, name: name == "second"):
    ...
    def test_selection_changed_during_fetch_is_not_overridden(self):
        ...
        out = io.StringIO()
        with patch("xswap.manager.Manager", return_value=self.manager), patch("xswap.manager.read_limits", side_effect=read), \
             patch.object(Manager, "codex", return_value="codex"), \
             patch("xswap.switch.switch_running", return_value=REPORT) as broadcast, contextlib.redirect_stdout(out):
            code = main(["auto-tick"])
        self.assertEqual(code, 2)
    ...
    def test_a_selection_that_lands_after_the_guard_is_not_overwritten(self):
        ...
        import xswap.tick as xswap_tick
        self.add("second")
        self.add("third")
        self.enable("main,second,third")
        ...
        with patch("xswap.manager.Manager", return_value=self.manager), \
             patch("xswap.manager.read_limits", side_effect=fake), \
             patch.object(Manager, "codex", return_value="codex"), \
             patch("xswap.tick.decide", arming_decide), \
