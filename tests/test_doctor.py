...
class DoctorTests(unittest.TestCase):
    ...
    def test_codex_found_through_a_relative_path_entry_fails_without_running_it(self):
        ...
        with patch("shutil.which", side_effect=lambda n: "bin/codex" if n == "codex" else None), \
                patch("xswap.doctor.subprocess.run") as run:
    ...
    def test_openclaw_probe_strips_secret_env(self):
        ...
        with patch.dict(os.environ, {"CODEX_API_KEY": "leak"}), \
                ...
                patch("xswap.doctor.resolve_openclaw_package_root", return_value=package_root), \
                patch("subprocess.run", side_effect=fake_run):
...
class AutoSessionDoctorTests(unittest.TestCase):
    ...
    def running_session(self, name, state, running=True, version=None):
        from xswap.manager import __version__, private_dir
...
class WrapperPathTests(unittest.TestCase):
    ...
    def test_a_concurrent_reconnect_cannot_produce_a_row_whose_clauses_are_both_false(self):
        ...
        with patch("xswap.doctor.entry_drift", racing_drift), patch("xswap.codex_cli.entry_drift", racing_drift):
            row = self.wrapper_row(self.bin_a, self.bin_b)
        self.assertEqual(row["status"], "WARN")
