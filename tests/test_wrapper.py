...
class ReconnectWrapperTests(WrapperFixture):
    ...
    def test_swap_failure_keeps_the_recovery_record_and_names_the_manual_command(self):
        ...
        self.connect(proxy, cli)
        self.repoint(cli, updated)
        with patch('xswap.codex_cli.swap_symlink', side_effect=OSError(errno.EACCES, 'Permission denied')), \
    ...
    def test_a_failed_swap_leaves_no_temp_link_next_to_the_entry(self):
        ...
        with patch('xswap.codex_cli.os.replace', side_effect=refuse_the_temp_link), \
                contextlib.redirect_stderr(io.StringIO()) as err:
            self.assertIsNone(reconnect_wrapper(self.manager))
...
class LaunchExitReconnectTests(WrapperFixture):
    ...
    def launch(self, proxy, cli, serve):
        with self.which(proxy, cli), patch('xswap.codex_cli.serve_cli', serve), patch('xswap.plugins.ensure_plugins'), \
    ...
    def test_reconnects_even_when_the_bridge_fails(self):
        ...
        with self.which(proxy, cli), patch('xswap.codex_cli.serve_cli', failing_serve), \
                patch('xswap.plugins.ensure_plugins'), contextlib.redirect_stdout(io.StringIO()), \
                contextlib.redirect_stderr(io.StringIO()) as err:
            with self.assertRaises(LiveError):
...
class PassthroughExitReconnectTests(WrapperFixture):
    ...
    def run_passthrough(self, proxy, cli, call):
        with patch('xswap.manager.Manager', return_value=self.manager), self.which(proxy, cli), \
                patch('subprocess.call', side_effect=call), patch('xswap.plugins.ensure_plugins'), \
                contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()) as err:
