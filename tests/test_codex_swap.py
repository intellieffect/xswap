...
class AccountTests(unittest.TestCase):
    ...
    def test_numbered_switch_keeps_slots_and_broadcasts_resolved_name(self):
        ...
        with patch("xswap.manager.Manager", return_value=self.manager), patch("xswap.switch.switch_running", return_value={"failed": 0, "unconfirmed": 0}) as broadcast, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_invalid_or_disabled_slot_does_not_change_selection(self):
        ...
        with patch("xswap.manager.Manager", return_value=self.manager), patch("xswap.switch.switch_running") as broadcast, contextlib.redirect_stderr(io.StringIO()):
    ...
    def test_use_preserves_numeric_account_names(self):
        ...
        with patch("xswap.manager.Manager", return_value=self.manager), contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_app_uses_same_account_home_and_separate_cookie_directory(self):
        ...
        app.mkdir()
        with patch("xswap.manager.sys.platform", "darwin"), patch("xswap.manager.subprocess.run", return_value=subprocess.CompletedProcess([], 0)) as run:
    ...
    def test_openclaw_reload_is_required_before_changing_active_account(self):
        ...
        with patch("xswap.manager.shutil.which", side_effect=lambda name: executable if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", side_effect=responses):
    ...
    def test_openclaw_dry_run_does_not_reload_or_select(self):
        self.manager.register("main")
        executable = self.bridge_installation()
        with patch("xswap.manager.shutil.which", side_effect=lambda name: executable if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", return_value=subprocess.CompletedProcess([], 0, '{"dryRun":true}', "")) as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_openclaw_success_changes_default_after_reload(self):
        ...
        with patch("xswap.manager.shutil.which", side_effect=lambda name: executable if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", side_effect=responses), contextlib.redirect_stdout(io.StringIO()):
            self.manager.sync_openclaw("second", select=True)
    ...
    def test_openclaw_sync_refuses_node_and_openclaw_found_through_a_relative_path_entry(self):
        ...
        for name, found in (("node", "bin/node"), ("openclaw", "bin/openclaw")):
            with self.subTest(name=name):
                ...
                with patch("xswap.manager.shutil.which", side_effect=which), \
                        patch("xswap.manager.subprocess.run") as run:
    ...
    def test_login_calls_codex_with_selected_home(self):
        self.manager.register("main")
        with patch("xswap.manager.subprocess.call", return_value=0) as call, \
    ...
    def test_login_refreshes_quota_cache_and_reauthenticates_sessions_on_that_account(self):
        ...
        with patch("xswap.manager.subprocess.call", return_value=0), \
             patch("xswap.manager.read_limits", return_value=response()), \
             patch("xswap.switch.switch_running", return_value=report) as signal, \
    ...
    def test_login_reports_usage_failure_without_hiding_the_signin(self):
        self.manager.register("main")
        with patch("xswap.manager.subprocess.call", return_value=0), \
             patch("xswap.manager.read_limits", side_effect=UsageError("usage service unavailable")), \
             patch("xswap.switch.switch_running", return_value=dict(applied=0, pending=0, unsupported=0, failed=0, unconfirmed=0)), \
             patch.object(Manager, "codex", return_value="codex"), \
             contextlib.redirect_stdout(io.StringIO()) as out:
            self.assertEqual(self.manager.login("main"), 0)
        text = out.getvalue()
    ...
    def test_failed_login_does_not_touch_cache_or_sessions(self):
        self.manager.register("main")
        with patch("xswap.manager.subprocess.call", return_value=1), \
             patch("xswap.manager.read_limits") as fetch, \
             patch("xswap.switch.switch_running") as signal, \
             patch.object(Manager, "codex", return_value="codex"), \
             contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_login_clears_a_rejected_login_record(self):
        ...
        with patch("xswap.manager.subprocess.call", return_value=0), \
             patch("xswap.manager.read_limits", return_value=response()), \
             patch("xswap.switch.switch_running", return_value=report), \
             patch.object(Manager, "codex", return_value="codex"), \
             contextlib.redirect_stdout(io.StringIO()) as out:
            self.assertEqual(self.manager.login("main"), 0)
    ...
    def test_login_the_service_still_rejects_records_a_fresh_failure(self):
        ...
        self.manager.remember_auth_failure("main", label, failed_at=1000)
        with patch("xswap.manager.subprocess.call", return_value=0), \
             patch("xswap.manager.read_limits", side_effect=UsageError("sign in again to read usage")), \
             patch("xswap.switch.switch_running", return_value=dict(applied=0, pending=0, unsupported=0, failed=0, unconfirmed=0)), \
             patch.object(Manager, "codex", return_value="codex"), \
             contextlib.redirect_stdout(io.StringIO()) as out:
            self.assertEqual(self.manager.login("main"), 0)
    ...
    def test_add_use_selects_account(self):
        ...
        with patch.dict(os.environ, env), \
             patch.object(Manager, "codex", return_value="/usr/bin/codex"), \
             patch("xswap.manager.subprocess.call", side_effect=self._write_login), \
             contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_use_without_bridges_says_so_instead_of_dumping_zero_counters(self):
        from xswap.manager import describe_switch_report
        ...
        from xswap.manager import describe_login_report
    ...
    def test_failed_login_does_not_select(self):
        ...
        with patch.dict(os.environ, env), \
             patch.object(Manager, "codex", return_value="/usr/bin/codex"), \
             patch("xswap.manager.subprocess.call", return_value=1), \
             contextlib.redirect_stdout(io.StringIO()):
            code = main(["add", "work"])
        self.assertEqual(code, 1)
        self.assertIsNone(self.manager.read()["active"])
    ...
    def test_add_use_with_aborted_browser_login_reports_error_only(self):
        ...
        with patch.dict(os.environ, env), \
             patch.object(Manager, "codex", return_value="/usr/bin/codex"), \
             patch("xswap.manager.subprocess.call", return_value=0), \
    ...
    def test_bridge_error_does_not_forward_raw_stderr(self):
        self.manager.register("main")
        executable = self.bridge_installation()
        with patch("xswap.manager.shutil.which", side_effect=lambda name: executable if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", return_value=subprocess.CompletedProcess([], 1, "", "fake-secret-token")):
    ...
    def test_pool_request_carries_every_account_and_keeps_single_path_keys(self):
        ...
        with patch("xswap.manager.shutil.which", side_effect=lambda name: executable if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", side_effect=responses) as run, contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_remove_purge_refusal_reaches_the_command_before_it_offers_to_delete(self):
        ...
        data["accounts"]["second"]["home"] = str(elsewhere)
        atomic_json(self.manager.registry, data)
        with patch("xswap.manager.Manager", return_value=self.manager), patch("builtins.input") as prompt, \
             contextlib.redirect_stderr(io.StringIO()) as err:
    ...
    def test_map_explicit_name_overrides_mapping(self):
        ...
        with patch("xswap.manager.os.getcwd", return_value=str(mapped)):
    ...
    def test_openclaw_never_follows_directory_mapping(self):
        ...
        self.manager.set_disabled("second", True)
        with patch("xswap.manager.os.getcwd", return_value=str(mapped_dir)), \
             patch("xswap.manager.shutil.which", return_value=None):
    ...
    def test_sync_openclaw_refuses_disabled_account_before_any_subprocess(self):
        ...
        atomic_json(second / "auth.json", self.auth)
        self.manager.set_disabled("second", True)
        with patch("xswap.manager.shutil.which") as which, patch("xswap.manager.subprocess.run") as run:
            with self.assertRaisesRegex(SwapError, "second is disabled. Run: xswap enable second"):
                self.manager.sync_openclaw("second", select=True)
            with self.assertRaisesRegex(SwapError, "second is disabled. Run: xswap enable second"):
...
class MainCLITests(unittest.TestCase):
    ...
    def test_use_on_rejected_login_returns_error_via_main(self):
        ...
        with patch("xswap.switch.switch_running") as broadcast, contextlib.redirect_stderr(io.StringIO()) as err:
            self.assertEqual(self.codex_swap.main(["use", "second"]), 1)
        broadcast.assert_not_called()
    ...
    def test_list_cached_reports_a_rejected_login_without_spawning_codex_via_main(self):
        manager = self.codex_swap.Manager()
        manager.remember_auth_failure("second", identity(manager.account("second")[1]))
        with patch("xswap.manager.read_limits", return_value=response()) as fetch, \
...
class ParsePoolTests(unittest.TestCase):
    def test_splits_trims_and_dedupes_in_order(self):
        from xswap.manager import parse_pool
...
class OpenclawPoolCliTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with patch.dict(os.environ, self.env):
            from xswap.manager import Manager as _Manager
    ...
    def run_main(self, argv):
        ...
        with patch.dict(os.environ, self.env), patch("xswap.manager.shutil.which", return_value=None), \
...
class SameOrgGuardTests(unittest.TestCase):
    ...
    def test_same_organization_pool_passes_the_guard(self):
        ...
        responses = [subprocess.CompletedProcess([], 0, json.dumps(result), ""), subprocess.CompletedProcess([], 0, '{"ok":true,"warningCount":0}', "")]
        with patch("xswap.manager.shutil.which", side_effect=lambda name: str(executable) if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", side_effect=responses), contextlib.redirect_stdout(io.StringIO()):
            self.manager.sync_openclaw(["main", "second"])
    ...
    def test_allow_mixed_bypasses_the_organization_guard(self):
        ...
        result = {"completed": ["main"], "backup": "/example/backup", "profileIds": ["p1", "p2"], "profileId": "p1"}
        responses = [subprocess.CompletedProcess([], 0, json.dumps(result), ""), subprocess.CompletedProcess([], 0, '{"ok":true,"warningCount":0}', "")]
        with patch("xswap.manager.shutil.which", side_effect=lambda name: str(executable) if name == "openclaw" else "/usr/bin/node"), patch("xswap.manager.subprocess.run", side_effect=responses) as run, contextlib.redirect_stdout(io.StringIO()):
            self.manager.sync_openclaw(["main", "second"], allow_mixed=True)
...
class ClearCooldownManagerTests(unittest.TestCase):
    ...
    def test_no_stale_cooldowns_reports_and_never_touches_the_gateway(self):
        ...
        self.write_db({})
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run") as run, contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_gateway_stop_failure_aborts_before_any_write(self):
        ...
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run", return_value=stop_failure) as run, \
                contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_successful_clear_stops_writes_and_restarts_the_gateway(self):
        ...
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run", return_value=ok) as run, \
                contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_without_yes_and_no_tty_requires_confirmation(self):
        ...
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run") as run, patch("sys.stdin.isatty", return_value=False), \
                contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_gateway_start_failure_after_write_is_reported_with_backup_path(self):
        ...
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run", side_effect=[ok, start_failure]), \
                contextlib.redirect_stdout(io.StringIO()):
    ...
    def test_clear_cooldown_refuses_an_openclaw_found_through_a_relative_path_entry(self):
        ...
        before = db.read_bytes()
        with patch("xswap.manager.shutil.which", return_value="bin/openclaw"), \
                patch("xswap.manager.subprocess.run") as run, contextlib.redirect_stdout(io.StringIO()):
            with self.assertRaises(SwapError) as refused:
                self.manager.clear_openclaw_cooldown("main", yes=True)
        run.assert_not_called()
        self.assertEqual(db.read_bytes(), before)
    ...
    def test_omitted_name_checks_every_registered_account(self):
        ...
        ok = subprocess.CompletedProcess([], 0, "", "")
        with patch("xswap.manager.shutil.which", return_value="/fixture/openclaw"), \
                patch("xswap.manager.subprocess.run", return_value=ok), contextlib.redirect_stdout(io.StringIO()):
...
class CachedFlagCLITests(unittest.TestCase):
    ...
    def test_list_and_usage_accept_valid_cached_seconds(self):
        with patch("xswap.manager.read_limits", return_value=response()), patch.object(self.codex_swap.Manager, "codex", return_value="codex"), contextlib.redirect_stdout(io.StringIO()) as out:
    ...
    def test_use_best_cached_reuses_a_fresh_cache_entry_without_spawning_codex(self):
        ...
        with patch("xswap.manager.read_limits", return_value=response()) as fetch, patch.object(self.codex_swap.Manager, "codex", return_value="codex"), \
             contextlib.redirect_stdout(io.StringIO()) as out:
            code = self.codex_swap.main(["use", "--best", "--cached", "60"])
        self.assertEqual(code, 0)
        self.assertIn("Selected main", out.getvalue())
        self.assertEqual(fetch.call_count, 2)
        with patch("xswap.manager.read_limits", return_value=response()) as fetch2, patch.object(self.codex_swap.Manager, "codex", return_value="codex"), \
...
class AutoLauncherTests(unittest.TestCase):
    ...
    def test_auto_launcher_uses_shared_runtime_and_explicit_pool(self):
        ...
        with patch('xswap.manager.sys.platform', 'darwin'), \
             patch('xswap.manager.shutil.which', side_effect=lambda name: str(proxy if name=='xswap-proxy' else executable)), \
             patch('xswap.manager.subprocess.run', return_value=subprocess.CompletedProcess([],0)) as run:
    ...
    def test_auto_launcher_refuses_an_xswap_proxy_found_through_a_relative_path_entry(self):
        ...
        executable.touch()
        with patch('xswap.manager.sys.platform', 'darwin'), \
             patch('xswap.manager.shutil.which',
    ...
    def test_a_second_launch_creating_the_same_tooling_link_is_not_a_failure(self):
        ...
        with patch('xswap.manager.sys.platform', 'darwin'), \
             ...
             patch('xswap.manager.subprocess.run', return_value=subprocess.CompletedProcess([], 0)), \
             contextlib.redirect_stdout(io.StringIO()):
            self.manager.launch_auto_app('main,second', str(app))
    ...
    def test_auto_dry_run_does_not_create_runtime(self):
        ...
        with patch('xswap.manager.sys.platform','darwin'),patch('xswap.manager.shutil.which',return_value='/fake/executable'),contextlib.redirect_stdout(io.StringIO()):
...
class BestAccountTests(unittest.TestCase):
    ...
    def test_picks_the_highest_headroom(self):
        ...
        with patch("xswap.manager.read_limits", side_effect=fake), patch.object(Manager, "codex", return_value="codex"):
    ...
    def test_all_unknown_falls_back_to_active_with_warning(self):
        ...
        with patch.dict(os.environ, env, clear=False), \
             patch("xswap.manager.read_limits", side_effect=fake), \
             patch.object(Manager, "codex", return_value="/usr/bin/codex"), \
             patch("xswap.manager.subprocess.call", return_value=0) as call, \
    ...
    def test_best_account_uses_cache_when_fresh_and_still_ranks_correctly(self):
        ...
        with patch("xswap.manager.read_limits", side_effect=fake) as fetch, patch.object(Manager, "codex", return_value="codex"):
    ...
    def test_run_best_dry_run_skips_a_rejected_login_without_spawning_codex(self):
        ...
        stdout = io.StringIO()
        with patch.dict(os.environ, env, clear=False), \
             patch("xswap.manager.read_limits", side_effect=fake) as fetch, \
             patch.object(Manager, "codex", return_value="/usr/bin/codex"), \
